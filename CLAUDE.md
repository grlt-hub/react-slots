...
- **server snapshot length-matched to the client first render:** `getServerCount → 0`; `getServerPresence → projectAllFalse(store.get())` (all-false over the current composition length, shared `EMPTY_PRESENCE` only when empty). Hydration-safe: server `[false,…]` of length N equals the client's first-render snapshot (no effect has run yet). The post-mount `false → true` settle is an effect-driven update, not a hydration mismatch.
- **observer is `{ childList: true, subtree: true }`** — `subtree` is required so a deep `null↔content` flip under an always-present wrapper element re-reports. The probe always reports the span's own `childNodes.length > 0`, so `subtree` only widens _when_ we re-check, never _what_ we count.
...
- Three distinct memo shapes, specialized at insert-time (never branch inside render):
  ...
  - with `filter` + `mapProps`: the double-barrier wrapper grows an update gate in its body — `if (filter(props)) last.current = <Memoized {...mapProps(props)} />; return last.current` with `last = useRef`.
...
- Freeze mechanics: the cached element lives in `useRef` inside the wrapper body, NOT in an insert-time closure (two mounted Roots of the same slot must freeze independently), and the freeze is an element-identity bail — returning the same element reference makes React skip reconciling the `Memoized` subtree. Do NOT move the gate into memo's second argument: a comparator can't gate the first render, which is a hole in the narrowing guarantee.
...
- **Presence (opt-in) is a FULL FORK by owner decision:** `createSlot(config?)` opens with `if (!config?.presence) return <non-presence impl>` whose body is **byte-identical to HEAD** (provable by diff, modulo the one enclosing-block indentation level oxfmt owns). Non-presence slots pay **zero** render-path cost and allocate nothing presence-related. `createSlot` is now a **function declaration** (overloads) — the single deliberate house-style exception (an arrow can't carry overload signatures).
...
9. **Opt-in cost:** per child +1 `display:contents` span, +1 `MutationObserver` (childList+subtree), +1 layout effect, +1 `queueMicrotask` per coalesced burst; per slot one internal slot-store subscription. Non-presence slots pay nothing (full fork).
